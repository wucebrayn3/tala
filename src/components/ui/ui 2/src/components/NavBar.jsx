...
const NavBar = ({ isAuthenticated, onLogout }) => {
  ...
  const [loadingProfile, setLoadingProfile] = useState(true); // Loading state for profile info
      ...
      try {
        if (auth.currentUser) {
          ...
          if (docSnap.exists()) {
            ...
            setEmail(userData.email || auth.currentUser.email || "No Email");
          } else {
            console.log("No such document!");
            setFirstName("User");
            setEmail(auth.currentUser?.email || "No Email");
          }
        } else {
          setFirstName("User");
          setEmail("No Email");
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        studyToolsRef.current &&
        !studyToolsRef.current.contains(event.target)
      ) {
        setStudyToolsOpen(false);
      }
    };
      ...
      <ul style={styles.navLinks}>
        ...
        <li style={styles.navLinksLi} ref={dropdownRef}>
          <button style={styles.profileButton} onClick={toggleDropdown}>
            <img
              src={storedImage || "/Images/profile.png"}
              alt="Profile"
          ...
          {dropdownOpen && (
            <div style={{ ...styles.profileDropdown, display: "block" }}>
              <div style={styles.profileInfo}>
                <img
                  src={storedImage || "/Images/profile.png"} // fallback to default if not set
                  alt="Profile"
                  style={styles.dropdownProfileIcon}
                />

                <div style={styles.profileDetails}>
                  {/* Placeholder user data */}
                  <p style={styles.profileName}>{firstName}</p>
